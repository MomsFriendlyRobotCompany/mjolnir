...
void norm4(float &x, float &y, float &z, float &w){
...
Madgwick::Madgwick(const float b): beta(b) {
    q[0] = 1.0;
    q[1] = 0.0;
    q[2] = 0.0;
    q[3] = 0.0;
...
void Madgwick::getEuler(float &roll,float &pitch,float &yaw){
...
void eCompass::update(float ax, float ay, float az, float mx, float my, float mz){
    ...
    else if (yaw < 0.0) yaw += 2.0*PI;
}

void eCompass::getEuler(float &r,float &p,float &y){
    r = roll*rad2deg;
    p = pitch*rad2deg;
    y = yaw*rad2deg;
}
