...
class FeatureDetector:
    def __init__(self, method):
        if method == 0:
            self.fd = cv2.FastFeatureDetector_create()
            self.fd.setNonmaxSuppression(True)
            self.fd.setThreshold(5)  # 20
        elif method == 1:
            self.fd = cv2.ORB_create()
        elif method == 2:
            self.fd = ShiTomasi()

    def detect(self, im):
        keypoints = self.fd.detect(im,None)
        if not isinstance(keypoints, np.array):
